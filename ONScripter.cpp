...
static SDL_Surface* flipped_surface = NULL;
int ONScripter::init()
{
    initSDL();
    openAudio();
...
#if defined(MIYOO)
SDL_Surface*surface=accumulation_surface;
flipped_surface = SDL_CreateRGBSurface(SDL_SWSURFACE,
	                               surface->w,surface->h,
...
#if defined(MIYOO)
//see https://github.com/rohit-n/Clonepoint/blob/master/texture.cpp
...
SDL_Surface* flipSurface2(SDL_Surface* surface, FlipType ft, SDL_Rect *dst_rect)
{
	int x, y;
	SDL_LockSurface(surface);
	SDL_LockSurface(flipped_surface);
	...
	if (ft == Flip_Vertical)
	{
		for(y = 0; y < surface->h; y++)
		{
if (dst_rect && y < dst_rect->y && y >= dst_rect->y + dst_rect->h) continue;
			for(x = 0; x < surface->w; x++)
			{
if (dst_rect && x < dst_rect->x && x >= dst_rect->x + dst_rect->w) continue;
				putpixel(flipped_surface, x, y, getpixel(surface, surface->w - x - 1, surface->h - y - 1));
			}
		}
	}
	else
...
void flipSurface3(SDL_Surface* surface, SDL_Surface* flipped_surface, FlipType ft)
{
	int x, y;
	SDL_LockSurface(surface);
	SDL_LockSurface(flipped_surface);
...
#if defined(MIYOO)
    SDL_Rect dst_rect2 = {screen_device_width - (rect.x + rect.w) - 1, screen_device_height - (rect.y + rect.h) - 1, rect.w, rect.h};
SDL_BlitSurface( flipSurface2(accumulation_surface, Flip_Vertical, &dst_rect), &dst_rect2, screen_surface, &dst_rect2 );
//flipSurface3(accumulation_surface, screen_surface, Flip_Vertical);
    SDL_UpdateRect( screen_surface, dst_rect2.x, dst_rect2.y, dst_rect2.w, dst_rect2.h );
#else
